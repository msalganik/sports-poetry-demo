...
- Upon confirmation, Claude writes timestamped config file in `output/configs/`
...
- **Claude**:
  ...
  - Writes timestamped config file to `output/configs/`
  - Launches `orchestrator.py`
  - Monitors progress

- **Python (`orchestrator.py`)**:
  - Reads config file (via `--config` flag)
...
└── output/
    ├── configs/
    │   └── config_TIMESTAMP.json  # Generated by create_config skill
...
4. Claude writes timestamped config file to `output/configs/config_TIMESTAMP.json` with:
   - `sports`: list of sport names
   - `retry_enabled`: boolean flag
   - `generation_mode`: template or llm
   - (session_id and timestamp are auto-generated at runtime by orchestrator)

### Phase 2: Orchestrator Initialization
1. Claude launches `orchestrator.py --config output/configs/config_TIMESTAMP.json`
2. Orchestrator reads config file
...
1. ✅ Timestamped config file in `output/configs/` with validated sports list
